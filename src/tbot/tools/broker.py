...
from tbot.models.schemas import Direction, OrderStatus, Position
...
def broker() -> Any:
    if settings.meta_api_token:
        try:
            from tbot.tools.metaapi_broker import MetaApiBroker
            return MetaApiBroker()
        except Exception as exc:
            import logging
            logging.getLogger(__name__).warning("Failed to init MetaApiBroker: %s", exc)
    return MockBroker()
...
class MockBroker:
    ...
    async def place_order(
        ...
        filled_price = self._apply_slippage(
            position.entry_price, position.signal.direction, slippage
        )
